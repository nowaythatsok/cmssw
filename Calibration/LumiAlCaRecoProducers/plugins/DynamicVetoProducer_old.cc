...
class DynamicVetoProducer_old : public DQMOneEDAnalyzer<edm::one::WatchLuminosityBlocks> {
public:
  explicit DynamicVetoProducer_old(const edm::ParameterSet&);
  ~DynamicVetoProducer_old() override;
...
double DynamicVetoProducer_old::getStd(const std::map<int, unsigned int>& moduleID2value, const double mean){
  double sum2 = 0;
  for (const auto& [key, value] : moduleID2value) {
...
std::vector<double> DynamicVetoProducer_old::getQuantile(const std::vector<double>& inData, const std::vector<double>& probs)
{
...
std::vector<double> DynamicVetoProducer_old::getQuantile(const std::map<int, double >& inData, const std::vector<double>& probs)
{
  if (inData.empty() || probs.empty()) return std::vector<double>();
...
TrackerRegion DynamicVetoProducer_old::getTrackerRegion(unsigned int mId){
...
//--------------------------------------------------------------------------------------------------
int DynamicVetoProducer_old::addBadModules(const std::map<int, double>& moduleID2value, const double center, const double distance,  std::vector<int>& badModules){
  int countBad = 0;
...
void DynamicVetoProducer_old::beginLuminosityBlock(edm::LuminosityBlock const& lumiSeg, const edm::EventSetup& iSetup) {
  lumisectionCount_++;
}

void DynamicVetoProducer_old::endLuminosityBlock(edm::LuminosityBlock const& lumiSeg, const edm::EventSetup& iSetup) {
...
//--------------------------------------------------------------------------------------------------
// void DynamicVetoProducer_old::dqmEndRun(edm::Run & runSeg, const edm::EventSetup& iSetup) {
void DynamicVetoProducer_old::dqmEndRun(const edm::Run & runSeg, const edm::EventSetup& iSetup) {
  ...
  edm::LogInfo("INFO") << "DynamicVetoProducer_old::dqmEndRun: Number of Lumisections processed in run " << runSeg.run() << " : " << lumisectionCount_;
  std::cout << "DynamicVetoProducer_old::dqmEndRun: Number of Lumisections processed in run " << runSeg.run() << " : " << lumisectionCount_ << std::endl;
  ...
  // edm::LogInfo("INFO") << "DynamicVetoProducer_old::dqmEndRun: Modules removed in round 1: " << additionalVeto1_.size();
  std::cout << "DynamicVetoProducer_old::dqmEndRun: Modules removed in round 1: " << additionalVeto1_.size() << std::endl;
  ...
  // edm::LogInfo("INFO") << "DynamicVetoProducer_old::dqmEndRun: Modules removed in round 1: " << additionalVeto2_.size();
  std::cout << "DynamicVetoProducer_old::dqmEndRun: Modules removed in round 1: " << additionalVeto2_.size() << std::endl;
  ...
  if (saveCSVFile_) {
    ...
    csfile.close();
    // edm::LogInfo("INFO") << "DynamicVetoProducer_old::dqmEndRun: CSV created.";
  }
  ...
  if (poolDbService.isAvailable()) {
    ...
    edm::LogInfo("INFO") << "DynamicVetoProducer_old::dqmEndRun: PoolDBService required.";
...
void DynamicVetoProducer_old::endJob() {
  edm::LogInfo("INFO") << "DynamicVetoProducer_old::endJob: Executing.";
}

DEFINE_FWK_MODULE(DynamicVetoProducer_old);
